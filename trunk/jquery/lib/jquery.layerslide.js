...
slideEffects:{
	...
	//vertical slide Down
	down: function($this, $first, $second){return {mOver:{top: $this.height() + 'px'},mOut:{top: 0}}},
	//vertical slide Raise
	raise: function($this, $first, $second){return {mOver:{top:-$this.height() + 'px'},mOut:{top:0}}},
	//horizontal slide Left	
	left: function($this, $first, $second){return {mOver:{left:-$this.width() + 'px'},mOut:{left:0}}},
	//horizontal slide Right
	right: function($this, $first, $second){return {mOver:{left:$this.width() + 'px'},mOut:{left:0}}},
	
	
	//diagonal slide TopLeft
	topLeft:function($this, $first, $second){return {mOver:{top:-$this.height() + 'px', left:-$this.width() + 'px'},mOut:{top:0, left:0}}},
	//diagonal slide TopRight
	topRight:function($this, $first, $second){return {mOver:{top:-$this.height() + 'px', left:$this.width() + 'px'},mOut:{top:0, left:0}}},
	//diagonal slide BottomLeft
	bottomLeft:function($this, $first, $second){return {mOver:{top:$this.height() + 'px', left:-$this.width() + 'px'},mOut:{top:0, left:0}}},
	//diagonal slide BottomRight
	bottomRight:function($this, $first, $second){return {mOver:{top:$this.height() + 'px', left:$this.width() + 'px'},mOut:{top:0, left:0}}},
	...
	partialDown: function($this, $first, $second){
		...
		var h = $second.height();
		$second.css({position: 'static'});
		
		return {mOver:{top:h + 'px'},mOut:{top:0}};
	},
	//vertical slide partial Raise.
	partialRaise: function($this, $first, $second){
		$first.css({zIndex:9999});
		$second.css({position: 'absolute', zIndex:9998});
		$second.css({top: $this.height() - $second.height() + 'px'});

		return {mOver:{top:-$second.height() + 'px'},mOut:{top:0}};
	},
	//vertical slide partial Left.
	partialLeft: function($this, $first, $second){
		$first.css({zIndex:9999});
		$second.css({position: 'absolute', zIndex:9998});
		$second.css({left: $this.width() - $second.width() + 'px'});
		
		return {mOver:{left:-$second.width() + 'px'},mOut:{left:0}};
	},
	...
	secondRaise:function($this, $first, $second){
		$second.css({top: $this.height() + 'px'});		
		return {mOver:{top:$this.height() - $second.height() + 'px'},mOut:{top:$this.height() + 'px'}}; 
	},
	// Second Element vertical slide Down
	secondDown:function($this, $first, $second){
		$second.css({top: -$second.height() + 'px'}); //abusolute指定した後でtopを指定しないとfirefoxでtopの位置がおかしくなる。
		return {mOver:{top:0},mOut:{top:-$second.height() + 'px'}};
	},
	// Second Element horizontal slide from Left
	secondLeft:function($this, $first, $second){
		$second.css({left: -$second.width()});		
		return {mOver:{left:0},mOut:{left:-$second.width()}}; 
	},
	// Second Element horizontal slide from Right
	secondRight:function($this, $first, $second){
		$second.css({left:$this.width()});
		return {mOver:{left:$this.width() - $second.width()},mOut:{left:$this.width()}};
	},
	
	
	//Second Element diagonal slide TopLeft
	secondTopLeft:function($this, $first, $second){
		$second.css({top:-$second.height(), left:-$second.width()});
		return {mOver:{top:0, left:0},mOut:{top:-$second.height(), left:-$second.width()}};
	},
	//Second Element diagonal slide TopRight
	secondTopRight:function($this, $first, $second){
		$second.css({top:-$second.height(), left:$this.width(), width:$second.width(), height:$second.height()});
		return {mOver:{top:0, left:$this.width() - $second.width()},mOut:{top:-$second.height(), left:$this.width()}};
	},
	//Second Element diagonal slide BottomLeft
	secondBottomLeft:function($this, $first, $second){
		$second.css({top:$this.height(), left:-$second.width()});
		return {mOver:{top:$this.height() - $second.height(), left:0},mOut:{top:$this.height(), left:-$second.width()}};
	},
	//Second Element diagonal slide BottomRight
	secondBottomRight:function($this, $first, $second){
		$second.css({top:$this.height(), left:$this.width(), width:$second.width(), height:$second.height()});
		return {mOver:{top:$this.height() - $second.height(), left:$this.width() - $second.width()},mOut:{top:$this.height(), left:$this.width()}};
	}
}});
