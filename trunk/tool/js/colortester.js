...
$.extend({
	...
	prevSlctr:"#preview" //Preview Element Selecter
});
...
$.bindClickColorItem = function(){
	var
	keys = ['web-color', 'named-color'],
	slctr = ['td.' + keys[0] + '-item', 'td.' + keys[1] + '-item'].join(',');
	...
	$(slctr).click(function(){
		...
		if (attr != 'background-color' && attr != 'color') {
			...
			$('#apply-attr-inner').css({top: 0});
		}
		
		$($.prevSlctr).trigger('changeStyle', {'attr':attr, 'val':colorCode});

	});
	
	$($.prevSlctr).trigger('changeStyle', {'attr':'background-color', 'val':'#333333'});
	$($.prevSlctr).trigger('changeStyle', {'attr':'color', 'val':'#cccccc'});
}
...
$.bindClickFontFamilyItem = function(){
	...
	$($.prevSlctr).trigger('changeStyle', {'attr':key, 'val':'Tahoma'});
}
...
$.bindSelectApplyAttr = function(){
	...
	downRange = '+=' + parentHeight + 'px',
	upRange = '-=' + parentHeight + 'px',
	applyToggle = {
...
$.bindChangeFontSize = function(){
	...
	$('#' + key + '-select').change(function(){
			...
			$($.prevSlctr).trigger('changeStyle', {attr:key, val:fsize});
		}).trigger('change', this);
}
...
$.bindClickItemNavi = function(){
	$('a.navi-item').click(function(){
		...
		$('#wrap-' + $(this).text()).slideDown(400, function(){
			$.effecting = false;
		});
