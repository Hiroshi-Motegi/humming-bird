...
jQuery.fn.extend({
  positionCenter: function() {
    return $(this).css({
		...
		"top":Math.floor(($(window).height() - $(this).height()) / 2),// + $(window).scrollTop(),
		"left":Math.floor(($(window).width() - $(this).width()) / 2)// + $(window).scrollLeft()
	});
  }
});
...
var cl = {
	...
	bindClickColorItem:function(){
		...
		$(item_selector).bind("click", function(){
			...
			var code = jQuery.data(this, "color").code;
	...
	showCssText:function(){	
		...
		.bind("click", function(event){
			$(this).stop();
			$("#css-text").remove();
			$(this).fadeTo("normal", 0.0, function(){ $(this).remove(); });
			event.stopPropagation();
		});
	},
	positionCenter:function(elem){
		$(elem).css({
			"position":"fixed",
			"top":Math.floor(($(window).height() - $(elem).height()) / 2),// + $(window).scrollTop(),
			"left":Math.floor(($(window).width() - $(elem).width()) / 2)// + $(window).scrollLeft()
		});
	},
	...
	getPageSize:function(){   
		...
		return [Math.min(xScroll, windowWidth), Math.max(yScroll, windowHeight)];
	},
	init:function(){
		//三つでセット
