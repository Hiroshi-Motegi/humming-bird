...
xETitleElement.prototype = {
	...
	createAncElement: function(txt){
		sw = EDIT_STRING(txt);
		Anc = document.createElement("a");
		Anc.title = "Keyword:[" + Edit_PointUseAtTitle(sw) + "] Go Search";
		Anc.setAttribute("href", "javascript:OnLoad(\'" + Edit_PointUseAtHref(sw) + "\');");
		Anc.appendChild(document.createTextNode(txt));
		return Anc;
	},
	AddToTitleElement: function(obj){
		this.TitleElement.appendChild(obj);
	},
	createTitleElement: function(){
		...
		if (str.match(re)){
			...
			this.AddToTitleElement(document.createTextNode(rankNum));
		}
		...
		for (etcnt = 0; etcnt < arr_et.length; etcnt++) {
			if(etcnt != 0){
				this.AddToTitleElement(document.createTextNode(" - "));
			}
			if (arr_et[etcnt].match(/(^EP$|^Single$|^Various\s\s?Artists$)/)) {
				this.AddToTitleElement(document.createTextNode(arr_et[etcnt]));
			}else{
				this.AddToTitleElement(this.createAncElement(arr_et[etcnt]));
			}
		}
	}
};
			...
			for (var i = 0; i < result.feed.entries.length; i++) {
				...
				if (List_GetSelectedValue("EntryVisiblityList") == Content_Selected.Full) {
					...
					if (coverNodes != null) {
						...
						imgEl.setHrefToAnc(google.feeds.getElementsByTagNameNS(eNode, ns, "albumLink")[0].firstChild.nodeValue);
...
$(function(){
	...
	$('#Side-Wrap > h4.mnTitle').click(function(){
		var $tchld = $(this).children();
		if($(this).next().css("display") == "none"){
			$($tchld[0]).text("▼");$($tchld[2]).text("▼");
		}else{
			$($tchld[0]).text("▶");$($tchld[2]).text("◀");
		}
